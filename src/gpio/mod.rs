...
#[derive(Debug)]
pub enum Error {
    PinAlreadyInUse(PortNumber, PinNumber),
}
...
pub struct InputPin<'a, T> {
    pin: PinNumber,
    input_data: &'a T,
}

impl<'a> InputPin<'a, gpioa::IDR> {
...
pub struct OutputPin<'a, T, U> {
    pin: PinNumber,
    output_data: &'static T,
    bit_set_reset: BsrrRef<'a, U>,
}

impl<'a> OutputPin<'a, gpioa::ODR, gpioa::BSRR> {
    pub fn get(&self) -> bool {
        use self::PinNumber::*;
